...
Check the latest example sessions version from the [file list](https://a3s.fi/swift/v1/chipster-example-sessions/). Don't worry if the latest version there looks older than the latest Chipster version. It probably means only that the example sessions haven't changed since that version.
