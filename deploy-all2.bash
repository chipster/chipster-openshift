...
if [ $# -eq 0 ]
  then
    echo "Usgae: deploy.bash PROJECT DOMAIN IMAGE_SERVER"
    exit 0
fi
...
IMAGES_SERVER=${$2:-172.30.130.41:5000}
...
oc delete dc auth && oc delete service auth
oc new-app auth
...
# run in the container to fix dstat and other programs requiring a username
echo "chipster:x:$(id -u):$(id -g)::/tmp:/bin/bash" >> /etc/passwd

# download tools-bin
oc new-app base
oc set volume  dc/base --add -t pvc --mount-path /opt/chipster/tools --claim-name tools
oc rsh $(oc get pod -l app=base | grep Running | cut -f 1 -d " ") bash

pushd /opt/chipster/tools

curl http://vm0151.kaj.pouta.csc.fi/artefacts/create_tools_images/194/parts/files.txt | grep tar.lz4$ > files.txt
time cat files.txt | parallel -j16 "curl http://vm0151.kaj.pouta.csc.fi/artefacts/create_tools_images/194/parts/{} | lz4c -d | tar -x"

# Finally go to Configuration tab of each build which has a GitHub source, copy the Github webhook URL and paste it to GitHub's settings of the 
# repository. Disable SSL check in the webhook's settings.
